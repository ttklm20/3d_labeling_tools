...
class WorkSpace {
	...
	for(var i = 0; i < this.fileList.length; i++){
		this.results[i] = new Array(5);
	}
	this.originalSize[0] = 416; //800
	this.originalSize[1] = 505; //600
    ...
    getAnnotations() {
	if(this.labeling_files.indexOf(this.fileList[this.curFile]+this.camera_id) == -1){
	    this.labeling_files.push(this.fileList[this.curFile]+this.camera_id)
	    var res = [];
	    var fileName = this.fileList[this.curFile] + ".txt";
	    var rawFile = new XMLHttpRequest();
	    var allText;
	    rawFile.open("GET", this.workBlob + '/Annotations' + this.camera_id + '/' + fileName, false);
	    ...
	    this.loadAnnotations(this.results[this.curFile][parameters.camera_id]);
	}
    }
	...
	this.results[this.curFile][parameters.camera_id] = annotations;
    }
    ...
    // Archive and update database //sumple
    archiveWorkFiles() {
    	var annotations_result = this.results[1][2][0].x; //frame_id[0,...,frame_number], camera_id[0,...,4],tag_id[0,...,tag_number]
